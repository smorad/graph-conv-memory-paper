...
class EncoderPP(ObservationTransformer):
    ...
    def forward(self, obs):
        ...
        x = np.zeros((semantic.shape[0] + 1 + 1, *semantic.shape[1:]), dtype=np.float32) # Sem, sem_tgt, depth
...
def autoencoder_env(cls: NavEnv):

    class AutoencoderEnv(NavEnv):
        def pp_init(self):
            self.ae = CNNAutoEncoder()
        
        def forward(self, x):
            return self.ae(x)

    return AutoencoderEnv
